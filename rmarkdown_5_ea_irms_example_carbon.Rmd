...
# iso_files_w_peak_table %>% iso_get_peak_table() %>% view() look at the peak table situation

peak_table <- iso_files_w_peak_table %>% iso_get_peak_table()
file_info <- iso_files_w_peak_table %>% iso_get_file_info # look at what the files have inside
```
...
iso_files_w_file_info <- iso_files_w_peak_table %>% 
  ...
  iso_mutate_file_info(
    # what is the type of each analysis? case when makes a new column called type and gives it a type characterization
    type = case_when(
      id1 == "empty"      ~ "empty",
      id1 == "blank"      ~ "blank",
...
#easy way to plot just one file 
#iso_files_without_empty$`19605__pugel_73.dxf` %>% iso_plot_continuous_flow_data(peak_bounds = TRUE)
> 
```
...
peak_map <- 
  tibble::tribble(
    ~compound,     ~ref_nr, ~rt,
    # peak map data (row-by-row). defining the peaks you want it to label
    "CO2 analyte", NA,		  300,
    "CO2 ref",			1,	  	415,
    "CO2 ref",			2,	  	465
  )
...
iso_files %>% 
  ...
  # introduce a label column for coloring the lines s print f means you are printing a formatted thing
  ...
  iso_plot_continuous_flow_data(
    ...
    # peak labels for the analyte peak, isoformat is a wrapper for sprintf so you could also use the command there. %d for integers, %s for string, %.3f for float 
...
# going to create a new column called note so you can keep track of the randos
iso_files <- iso_files %>% 
...
calib_drift <- 
  ...
  iso_generate_calibration(
    ...
    model = c(
      ...
      loess(d13C_dev ~ file_datetime, span = 0.5) # adding a spline (i.e. blue line)
    ),
...
# spline technically looks the best, but it doesn't really improve the residuals so no need to add a drift correction
```
...
calib_linearity <- 
  peak_table_w_standards %>%
  # adding this here if you want to use a calibration on only part of the run, then make sure you add the group by in iso_prepare_for_calibration. if not splitting, remove this mutate and the group by. you can do this with time_range as well.
...
# or this with our split calibration 
calib_linearity_applied <- calib_linearity %>% filter(signal_range == "low area" & lin_calib =="lm(d13C_dev ~ 1)" | signal_range == "high area" & lin_calib == "lm(13C_dev ~ area44)") %>% 
  # apply calibration indication what should be calcculated
  iso_apply_calibration(predict = d13C_dev) %>% 
  # evaluate calibration range across area44
  iso_evaluate_calibration_range(area44)
